...
/*
 * File: Fig5.cpp
 * Description: This file provides the evolution of the probability difference of two chosen actions over time,
 * with the temperature set to tau = 0.5. Running the program can produce the results for Figure 5c 
 * in the article "Optimal coordination of resource: A solution from reinforcement learning."
 * One can modify the temperature in this program and select the corresponding time window 
 * to obtain other results from Figure 5 in the article.
 * Author: [Guozhong Zheng]
 * Date: [2024.11.3]
 * 
 * Usage: Compile and run the program. The output will be written to 
 * two text files:
 * -time_tau0.5.txt and delta_p0.5.txt. 
*/
...
int main()
{
 ...
 //   srand((unsigned)time(NULL));
    ...
    ofstream outfile("time_tau0.5.txt");
    if(!outfile){
        cout<<"You can't open the pointed fme!"<<endl;
        exit(1);
    }
    ofstream outfile2("delta_p0.5.txt");
    if(!outfile2){
        cout<<"You can't open the pointed fme!"<<endl;
        exit(1);
    }
    ...
    //--------- The evolution of the game by Q-learning -----------//
        ...
        //--------Choose an action------------//
            for(i=0;i<N;i++)
            {
				...
				outfile2<<it<<"  "<<pr[i]-prb[i]<<endl;   //Output delta p for each individual
            }
